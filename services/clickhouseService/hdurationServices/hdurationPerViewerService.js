...
exports.hdurationPerViewerService = async (startDate, endDate, filters) => {
  try {
    ...
    const cacheKey = `hduration_per_viewer_${startDate}_${endDate}_${filtersKeyPart}`;
    ...
    const query = `
      WITH viewer_counts AS (
        SELECT 
          ...
          SUM(toUInt32(abs(event_time_new - event_time))) AS total_hduration_seconds
        FROM analytics_events
        ${whereClause}
      )
      SELECT
        ROUND(
          if(unique_viewers = 0, 0, total_hduration_seconds / 60 / unique_viewers), 
          2
        ) AS hduration_per_viewer_in_mins
      FROM viewer_counts
    `;

    const params = { startDate, endDate };
    const result = await queryClickHouse(query, params);
    const value = result[0]?.hduration_per_viewer_in_mins || 0;
