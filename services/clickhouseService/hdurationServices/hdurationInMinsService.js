...
exports.hdurationInMinsService = async (startDate, endDate, filters) => {
  try {
    ...
    const cacheKey = `hduration_in_mins_${startDate}_${endDate}_${filtersKeyPart}`;
    ...
    const query = `
      SELECT 
        ROUND(SUM(toUInt32(event_time_new - event_time)) / 60, 2) AS hduration_in_mins
    ...
    const value = result[0]?.hduration_in_mins || 0;
