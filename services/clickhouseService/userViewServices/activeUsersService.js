...
exports.activeUsersService = async (startDate, endDate, filters) => {
  try {
    ...
    const cacheKey = `active_users_${startDate}_${endDate}_${filtersKeyPart}`;
    ...
    if (cachedData) {
      ...
      const activeUsers = Array.isArray(parsedCache)
        ? parseInt(parsedCache[0]?.active_users ?? 0)
        : parseInt(parsedCache?.active_users ?? 0);
    ...
    // Optional: store only the active_users count
